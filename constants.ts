...
- 전체 장식 강도는 1~10 단계 중 **3 단계** 수준으로 제한합니다.
- 사진 전체를 꽉 채우지 말고, 피사체 주변과 시선이 향하는 곳 위주로 **2~4개의 드로잉 포인트**만 배치합니다.
- 원본 사진의 빈 공간과 여백 감성을 어느 정도 유지해, "살짝 꾸민 포스터" 느낌을 목표로 합니다.
...
2. 맥락적 드로잉 추가(강도 3/10): [피사체의 특정 부위나 사물 이름]을 기준으로 최소 2곳, 최대 4곳에 [스타일] 느낌의 [구체적인 낙서 요소]를 추가하세요. (예: 고양이 귀 사이에 작은 하트 두 개, 스크래쳐 주변에는 진동하는 느낌의 물결선을 2~3줄 정도만 그려주세요.)
3. 배경 통합: [의자나 벽면]의 라인을 따라 [스타일] 드로잉을 가늘고 간격 있는 선으로 오버레이하여, 여백을 남기면서 실사와 그림이 하나로 어우러지게 하세요.
4. 최종 무드: 단순 필터가 아닌, 작가가 사진 위에 직접 펜으로 의미 있는 그림을 '꽉 채우지 않고 적당히' 그려 넣은 듯한 포스터 스타일로 생성하세요."
`;