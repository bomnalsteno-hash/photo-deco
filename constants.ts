...
2. 맥락적 드로잉 추가: [피사체의 특정 부위나 사물 이름] 위치에 [스타일] 느낌의 [구체적인 낙서 요소]를 추가하세요. (예: 고양이 귀 사이에 작은 하트를, 스크래쳐 주변에는 진동하는 느낌의 물결선을 그려주세요.)
3. 배경 통합: [의자나 벽면]의 라인을 따라 [스타일] 드로잉을 오버레이하여 실사와 그림이 하나로 어우러지게 하세요.
4. 최종 무드: 단순 필터가 아닌, 작가가 사진 위에 직접 펜으로 의미 있는 그림을 그려 넣은 듯한 포스터 스타일로 생성하세요."
`;